...
from views import *
from models import AchievementData, AchievementName
...
class Leaderboard(commands.Cog):
    ...
    async def showleaderboard(self, interaction: discord.Interaction, sort_by: Literal["name", "score", "achievements", "snipes", "times_sniped"] = "score", sort_direction: Literal["Highest First", "Lowest First"] = "Highest First"):
        ...
        table = []
        header = f"{'Name':<25} {'Snipes':>6}  {'Times sniped':>13} {'Ach. points':>13} {'Score':>6}"
        table.append(header)
        # print(table)
        table.append("-" * len(header))
        for name, snipes, sniped, achievements, score in rows:
            table.append(f"{name:<25} {snipes:>6.0f} {sniped:>13.0f} {achievements:>13.1f} {score:>6.1f}")
        # print(table)
        output = "```text\n" + "\n".join(table) + "\n```"
    ...
    async def playerstats(self, interaction: discord.Interaction, player: discord.Member):
        ...
        embed.add_field(name="**Current Killstreak**", value=str(player_data.kill_streak), inline=False)
